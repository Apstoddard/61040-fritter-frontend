 ...
 *
 * @param {HydratedDocument<Like>} follow - A follow
...
const constructFollowResponse = (follow: HydratedDocument<Follow>): FollowResponse => {
  ...
  const userUsername = followCopy.user.username;
  const followingUsername = followCopy.following.username;
  delete followCopy.user;
  delete followCopy.following;
  return {
    ...followCopy,
